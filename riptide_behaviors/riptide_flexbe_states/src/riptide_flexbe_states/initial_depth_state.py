...
class InitialDepthState(EventState):
	'''
	...
	def __init__(self, depth):
		...
		super(InitialDepthState, self).__init__(outcomes = ['failed', 'completed'])

		self.depth = depth #hardcode depth here
		...
		self._client = ProxyActionClient({self._topic: DepthAction}) # pass required clients as dict (topic: type)
	...
	def on_enter(self, userdata):
		...
		try:
			...
			Logger.logwarn('Failed to send the depth command:\n%s' % str(e))
			self._error = True
