...
class ImageProcessor:
    def __init__(self):
        ...
        self.pole_pub = rospy.Publisher("/task/pole", PoleData, queue_size=1)
        self.obj_pub = rospy.Publisher("/task/gate/alignment", ObjectData, queue_size=1)
        self.fwd_sub = rospy.Subscriber("/forward/image_raw", Image, self.image_callback, queue_size=1)
        self.bridge = CvBridge()
        self.prev_pos = list();

    def image_callback(self, data):
        if (self.gate_pub.get_num_connections > 0):
            self.obj_pub = rospy.Publisher("/task/gate/alignment", ObjectData, queue_size=1);
            self.process_gate(data)
        elif (self.pole_pub.get_num_connections > 0):
            self.obj_pub = rospy.Publisher("/task/pole/alignment", ObjectData, queue_size=1);
            self.process_pole(data)
