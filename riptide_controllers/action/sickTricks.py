...
class PlaySickTricks(object):
    ...
    def __init__(self):
        ...
        self._as = actionlib.SimpleActionServer(
            "sickTricks", riptide_controllers.msg.sickTricks, execute_cb=self.execute_cb, auto_start=False)
        self._as.start()
    ...
    def execute_cb(self, goal):
        ...
        switch (goal.maneuvers) {
            ...
            case 2: rospy.loginfo("Driving forward")
                    ...
                    rospy.loginfo("Turning right -90 degrees")
                    self.performActions(
                        self.yawAction(180, AttitudeCommand.POSITION)
                    )
                    rospy.info("Driving backward")
                    self.XPub.publish(-0.5, LinearCommand.VELOCITY)
                    rospy.sleep(3)
                    break
            case 3: rospy.loginfo("Going up and down while spining in z direction")
