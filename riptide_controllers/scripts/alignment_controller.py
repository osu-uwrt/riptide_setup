...
class AlignmentController():
    ...
    def bboxCb(self, msg):
        ...
        self.x_forceCmd = X_FORCE_P * self.x_error
        self.y_forceCmd = Y_FORCE_P * self.y_error
        self.z_forceCmd = Z_FORCE_P * self.z_error
        if self.cam == 0:
    ...
    def reconfigure(self, config, name):
        self.X_FORCE_P = config[name + "x_force_p"]
        self.Y_FORCE_P = config[name + "y_force_p"]
        self.Z_FORCE_P = config[name + "z_force_p"]
        self.MAX_FORCE = config[name + "max_force"]
